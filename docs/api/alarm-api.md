...
| 参数 | 类型 | 必填 | 默认 | 说明 |
|------|------|:----:|------|------|
| status | string | 否 | - | 告警状态过滤：`ACTIVE` / `ACKED` / `RESOLVED` |
| deviceId | int | 否 | - | 设备 ID 过滤 |
| alarmType | string | 否 | - | 告警类型：`SILENT_TIMEOUT` / `CONSECUTIVE_FAIL` |
...
| 401 | 未携带 token 或 token 失效 | Full authentication is required |
| 403 | VIEWER 角色访问 | Access is denied |
| 404 | 告警 ID 不存在 | Alarm not found: 42 |
...
| 404 | 告警 ID 不存在 | Alarm not found: {id} | 告警不存在或已被删除 |
| 409 | 告警状态不是 ACTIVE（已 ACKED 或 RESOLVED） | Cannot ack alarm in status RESOLVED | 该告警已确认或已恢复 |

---

### §1.4 POST /alarms/{id}/resolve — 手动恢复告警

- **完整路径**：`POST /api/v1/alarms/{id}/resolve`
- **角色**：仅 **ADMIN**
- **功能**：将 ACTIVE 或 ACKED 状态的告警手动标记为已恢复（RESOLVED）
- **请求 Body**：空（不需要 body）
...
| 403 | OPERATOR 或 VIEWER 角色调用 | Access is denied | 您没有此操作的权限 |
| 404 | 告警 ID 不存在 | Alarm not found: {id} | 告警不存在或已被删除 |
| 409 | 告警已是 RESOLVED 状态 | Already resolved | 该告警已恢复 |
...
- **角色**：ADMIN, OPERATOR
- **功能**：返回当前 ACTIVE 状态告警总数，供前端铃铛红点使用
...
- **角色**：ADMIN, OPERATOR
- **功能**：返回全厂设备健康状态概览，含在线/离线/告警/维保数量及告警最多的 Top 5 设备
...
> `topOffenders` 按 `ACTIVE + ACKED` 告警数降序排列，最多返回 5 条。
...
- **角色**：ADMIN, OPERATOR
- **功能**：获取系统级别的告警规则默认值（对所有未单独配置 override 的设备生效）
...
| maintenanceMode | boolean | 是否处于维保模式（维保期间抑制告警） |
...
| maintenanceMode | boolean | 是 | - | true = 抑制该设备告警 |
...
> 测试请求使用模拟告警：`{id: 0, deviceId: 0, code: "M-TEST"}`
...
|-------|------|---------|---------|------------|
| AlarmNotFoundException | 404 | 告警 ID 不存在 | `Alarm not found: {id}` | 告警不存在或已被删除 |
| AlarmStateException（ack 状态错误） | 409 | 对 ACKED/RESOLVED 告警调 ack | `Cannot ack alarm in status RESOLVED` | 该告警已确认或已恢复 |
| AlarmStateException（resolve 状态错误） | 409 | 对已 RESOLVED 告警调 resolve | `Already resolved` | 该告警已恢复 |
        ...
        - type: object
          properties:
            ...
            detail:
              ...
              description: 扩展字段 Map，结构依告警类型而异
        ...
        topOffenders:
          ...
          items:
            ...
            properties:
              ...
              activeAlarmCount:
                type: integer
                description: ACTIVE + ACKED 告警数之和
    ...
    DefaultsDTO:
      ...
      properties:
        ...
        suppressionWindowSeconds:
          type: integer
          description: 全局抑制窗口秒数（同一设备重复告警去抖）
...
- [告警配置参考](../product/alarm-config-reference.md)（阈值参数说明）
- [告警业务规则](../product/alarm-business-rules.md)（状态机、触发逻辑）
