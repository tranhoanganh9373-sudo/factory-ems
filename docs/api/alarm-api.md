...
### §1.7 GET /alarms/meter-status — 表计在线状态映射（v1.1.0+）

- **完整路径**：`GET /api/v1/alarms/meter-status`
- **角色**：ADMIN, OPERATOR, **VIEWER**（与 §1.5/§1.6 不同——表计列表页只读浏览也需要它）
- **功能**：返回每块仪表的实时采集状态，供 `/meters` 列表页直接联表渲染「采集状态」列。与 `/health-summary` 同口径（5 分钟 freshness 窗口 + 维护标记），但粒度到单表。

#### 响应 Schema

`Result<{ [meterId: number]: "ONLINE" | "OFFLINE" | "MAINTENANCE" }>`

| 状态值 | 触发条件 |
|---|---|
| `ONLINE` | `DiagnosticRingBuffer.lastGoodSampleAt` 在最近 5 分钟内 |
| `OFFLINE` | 5 分钟内无 GOOD 样本，且未处于维护模式（含从未上报样本的仪表）|
| `MAINTENANCE` | `alarm_rule_overrides.maintenance_mode = true`（独立分支，避免维护期被误计离线）|

> 注：响应包含**所有**仪表（不过滤 `enabled`）。前端按 `meter.enabled = false` 单独渲染「未启用」标签。
...
```bash
curl -X GET "http://localhost:8080/api/v1/alarms/meter-status" \
  -H "Authorization: Bearer <TOKEN>"
```
...
|------|---------|---------|
| 401 | 未携带 token 或 token 失效 | Full authentication is required |
| 403 | 非任何已知角色 | Access is denied |

#### 实现备注

后端入口 `AlarmController.meterStatus()`（`ems-alarm/src/main/java/com/ems/alarm/controller/AlarmController.java:88-95`）；状态计算逻辑 `AlarmServiceImpl.computeMeterOnlineStatuses()`（`ems-alarm/src/main/java/com/ems/alarm/service/impl/AlarmServiceImpl.java:176-193`）。`AlarmService.healthSummary()` 重构为复用此方法，确保 `/health-summary` 卡和 `/meter-status` 端点共享同一数据源。
