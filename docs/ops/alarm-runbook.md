...
| `suppression-window-seconds` | 300 | 报警抑制窗口（秒） |
...
4. 点击 **测试发送**：构造一条假报警立即同步发送，2xx 视为通过；否则页面显示返回 statusCode / error 提示（测试发送不写 delivery_log，不计重试次数）
5. 点击 **保存**
...
|---------|--------------|
| 检测逻辑 | 完全跳过，不触发新报警 |
| 已有 ACTIVE/ACKED 报警 | 保持原状（维护模式不自动关闭历史报警） |
| 站内通知 | 不发送 |
| Webhook | 不触发 |

**生效**：配置读取在每轮扫描时实时查询，最长延迟一个检测周期（≤ 60s）。

**关闭维护模式**：取消勾选并保存，下一轮检测（≤ 60s）立即恢复监控。维护期间错过的报警**不补发**；若关闭时设备仍处于故障状态，下一轮正常触发报警。
...
- 同设备同类型的 ACTIVE/ACKED 状态下**不会创建新报警**（去重）
- AUTO 恢复条件：数据恢复正常 + 距 `triggered_at` 超过 `suppressionWindowSeconds`（默认 5min）
- RESOLVED 后 5min 内不会再次触发同类型报警（抑制窗口防抖）
...
**两种报警类型（OR 关系，任一命中即触发；同轮 SILENT_TIMEOUT 优先）**：
...
| 同类型已有 ACTIVE/ACKED 报警 | 去重，不重复创建 |
...
- **报警中**：当前 `alarms` 表状态为 ACTIVE + ACKED 的设备数
- **维护中**：`alarm_rule_overrides.maintenance_mode = true` 的设备数
- **Top10**：ACTIVE + ACKED 数倒序，前 10 条（未解决报警最多的设备）
...
**alarm_inbox**：每用户每报警 1 行（ADMIN + OPERATOR），近 1 月单用户预期 < 1000 行。
...
当前版本**单实例运行**；多实例部署时每个实例独立执行检测，会重复创建报警（无分布式锁）。
...
| 限制 | 影响 | 计划版本 |
|------|------|---------|
| 监控范围仅限 collector 配置的设备 | 未在 collector 配置的设备无法报警 | v1.7+ |
...
| 现象 | 第一步排查 | 第二步 |
|------|-----------|--------|
| 用户没收到报警 | 角色检查（ADMIN/OPERATOR）+ 维护模式开关 + collector 配置是否含该设备 | 阈值是否过宽（silentTimeout / consecutiveFailCount）；查抑制窗口 |
| Webhook 一直失败 | UI → 下发流水 → `lastError` + `attempts` 字段 | 接收方日志 + 签名校验（Secret 是否一致）+ URL 可达性 |
| 报警一直 ACTIVE 不恢复 | 确认设备数据是否真实恢复（看 `meter_reading.ts` 或 collector status） | 是否在 5min 抑制窗内；可手动 ACK 后等待 AUTO 恢复 |
| 报警重复触发 / 频繁抖动 | 查抑制窗口配置（默认 300s）；查 alarms 历史行时间间隔 | 调大 `consecutive_fail_count` 或 `suppression-window-seconds` |
...
| 维护模式期间仍收到报警 | 确认报警 `triggered_at` 是否在维护模式生效前（历史遗留） | 查 `alarm_rules_override` 是否有记录且 `maintenance_mode=true` |
...
- **L2 模块 owner**：参考 spec §11.4 联系人列表；提供报警 ID、设备 meterCode、`webhook_delivery_log` 相关行截图
- **L3 紧急操作**：
