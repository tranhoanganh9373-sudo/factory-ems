...
本手册是 [installation-guide.md](./installation-guide.md) 的兜底参考。主流程 80% 场景跟着 guide 走就行，遇到下面这些问题再看本手册：
...
⚠️ 改 TZ 后，已有的时序数据时间戳是 UTC 写入，前后混用会让历史和新数据时区不一致。建议只在首次部署时定，后期不要改。
...
不要对外开放：5432（Postgres）、8086（InfluxDB）、8080（后端直连）、4318（Tempo OTLP）。这些都是内网联通用，对外放等于把 DB 放公网。
...
写入量估算公式：仪表数 × 24 小时 × (3600 / 轮询间隔秒)。轮询间隔 5 秒、500 仪表 = 500 × 24 × 720 ≈ 860 万条/天，按 5000 规模配置。
...
- 不要直接 `docker compose up -d --force-recreate frontend-builder`，它会用镜像里 bake 进去的 bundle（compose-up 那时的 HEAD），不是当前 git 工作树最新代码。除非要回滚。
