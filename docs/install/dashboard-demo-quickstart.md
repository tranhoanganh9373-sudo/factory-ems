...
**亮点**：上班前后负荷曲线明显分层（mock-data-generator 内置班次曲线）。
...
**亮点**：点 Top N 任一行 → 跳转仪表详情页，看那台表的近 7 天功率曲线 + 累计电量。
...
**亮点**：演示前最好准备一句话："你看，焊接车间的总入是 30%，但下面工序加起来只有 25%——剩下 5% 是漏检 / 公用区域损耗，这就是节能改造的切入点。"
...
| 听众 | 提示 |
|---|---|
| 老板 / 投资人 | 强调"这是装上 5 个工厂之后能看到的样子"，引导谈成本和 ROI |
| 客户工厂厂长 | 重点演示 Top N + Sankey + 平面图，"找耗能问题 + 内部分摊"两个价值点 |
| 客户 IT | 强调私有部署 + Docker + 标准协议（OPC UA / Modbus / MQTT），不绑定云 |
| 内部销售培训 | 让销售自己上手跑一遍 demo-up.sh，5 分钟跑通，建立信心 |
