...
| **办公楼 / 商超 / 数据中心 / 医院** | **跳过**——你的"产出"不是计件的，能效用 kWh/㎡ 或 kWh/床位 表达，而 ems-production 模块按"件数"建模 |
...
应该多出 `intensityKwhPerUnit` 字段（如果产品里有这一列）。**没有产量数据的节点这一列是 null——这就是为什么没产量时月度报表总有些空格**。
...
**这就是给老板看的 ROI 报告。看板/报表只展示原始数据，对比和换算最简单的做法是 Excel 拉出来手算，或者写 1 张 Grafana 仪表板**。
