...
| D | SLO + 告警规则 + promtool 测试 + SLO 总览 dashboard | D1–D5 | 1.5d | D5 → `docs/product/observability-slo-rules.md` |
...
- [ ] **Step 1: 在 ems-app/pom.xml 的 `<dependencies>` 节点末尾插入**
...
- [ ] **Step 1: 写失败测试 ObservabilityConfigTest.java**
...
- [ ] **Step 3: 实现 ObservabilityConfig.java**
...
- [ ] **Step 3: 实现 SchedulerInstrumentationAspect.java**
...
- [ ] **Step 1: 增加 management + tracing 配置**
...
- [ ] **Step 1: 撰文 — 内容来源 spec §11 完整照搬+落地说明**
...
- [ ] **Step 1: 写测试 CollectorMetricsTest**
...
- [ ] **Step 3: 在 ems-collector 现有 poll 入口注入**
...
- [ ] **Step 4: 跑全 ems-app + ems-collector 测试**
...
- [ ] **Step 1: 写测试 AlarmMetricsTest**（按 B1 模式：注册 + 至少 1 次记录 + 边界值）
- [ ] **Step 2: 实现 AlarmMetrics.java**（Gauge 用 `AtomicLong` map by type）
- [ ] **Step 3: 注入到 detector/dispatcher/service**：
...
- [ ] **Step 4: `./mvnw -pl ems-alarm,ems-app -am test`**
- [ ] **Step 5: Commit `feat(obs): AlarmMetrics 5 个指标 + ems-alarm 注入`**
...
- [ ] **Step 1-2: 测试 + 实现**（按 B1 模式）
- [ ] **Step 3: 入库 service 注入**：每条入库后 `metrics.incrementInsert(energyType)`；校验失败 `metrics.incrementDropped(reason)`；新增 `MeterLagJob`（@Scheduled fixedDelay 60s）调用 `meterRepository.findMaxLagSeconds()` → `metrics.setLag(seconds)`
- [ ] **Step 4: 测试 + 通过**
- [ ] **Step 5: Commit `feat(obs): MeterMetrics 3 个指标 + lag job + 入库注入`**
...
- [ ] **Step 1-2: 测试 + 实现**
- [ ] **Step 3: 注入**：GlobalExceptionHandler 兜底 `metrics.incrementException(ex.getClass().getSimpleName())`；AuditServiceImpl#write 后 `metrics.incrementAudit(action)`
- [ ] **Step 4: 测试 + 通过**
- [ ] **Step 5: Commit `feat(obs): AppMetrics 4 个指标 + 异常/审计注入`**
...
- [ ] **Step 1: 撰文 — 内容来源 spec §8**
  - 8.1 公共 labels
  - 8.2-8.5 各模块指标表（17 行）含名称/类型/单位/labels/含义/典型 PromQL/告警是否覆盖（指向 D Phase）
  - 8.6 cardinality 控制规则
  - 8.7 Spring Boot 已有指标列表（不重发明）
  - 附录：开发者新增指标 checklist（命名规范、label 上限、注册时机）
- [ ] **Step 2: 索引联动** docs/product/README.md
- [ ] **Step 3: Commit `docs(obs): metrics 字典（Phase B 落实）`**
...
- [ ] **Step 2: 写 `.env.obs.example`** — 字段完全对齐 spec §11.2 表（13 个变量），每行注释"必填/可选+默认+空值行为"。
...
- [ ] **Step 3: README.md** — 目录索引 + 快速启动命令 + 链回 spec/plan/runbook
- [ ] **Step 4: Commit `chore(obs): 观测栈目录骨架 + .env.obs.example`**
...
- [ ] **Step 2: 静态校验** — `docker run --rm -v $(pwd)/ops/observability/prometheus:/cfg prom/prometheus:v2.54.1 promtool check config /cfg/prometheus.yml`（rules 还没写，会报 0 条规则 OK）
- [ ] **Step 3: Commit `feat(obs): prometheus.yml 抓取配置（factory-ems + cadvisor + node-exporter）`**
...
- [ ] **Step 2: promtail-config.yml** — 抓 docker container stdout，按 container_name label 路由
...
- [ ] **Step 3: Commit `feat(obs): loki + promtail 配置`**
...
- [ ] **Step 1: tempo.yml**（接 OTLP HTTP 4318，本地 fs 存储，3d retention）
...
- [ ] **Step 2: alertmanager.yml** — 完整路由表（spec §9.4）+ 4 个 receiver（default-email / multi-channel-dingtalk / multi-channel-wechat / multi-channel-generic）+ 抑制规则。任一渠道 env 空 → receiver 配置空 url 静默跳过。
...
- [ ] **Step 4: Commit `feat(obs): tempo + alertmanager 配置 + 多通道路由`**
...
- [ ] **Step 3: Dockerfile（distroless 多阶段）**
...
- [ ] **Step 5: Commit `feat(obs): webhook-bridge Go 服务（钉钉/企微 适配 + HMAC 签名 + 重试）`**
...
- [ ] **Step 3: scripts/grafana-init.sh**
...
- [ ] **Step 6: chmod +x scripts/*.sh**

- [ ] **Step 7: 本地端到端冒烟（可选，防止合并 broken stack）**
...
- [ ] **Step 8: Commit `feat(obs): docker-compose.obs.yml + 启停脚本（含 grafana 密码自动生成）`**
...
- [ ] **Step 1: 撰文 — 来源 spec §16 + Phase C 全部产物的实操步骤**
...
8. 故障定位入口（指向 runbook）
- [ ] **Step 2: README 索引联动**
- [ ] **Step 3: Commit `docs(obs): deployment 指南（Phase C 落实）`**

---

## Phase D — SLO + 告警 + promtool 测试
...
> 每个 SLO 写 `recording_rules` 计算 SLI 与错误预算（30d 窗口），不直接告警；告警规则在 D2/D3/D4 单独写。

- [ ] **Step 1-4: 4 个 yml**（按 spec §9.1 PromQL）
...
- [ ] **Step 5: 静态校验** `promtool check rules ops/observability/prometheus/rules/slo-*.yml`
- [ ] **Step 6: Commit `feat(obs): 4 个 SLO 录制规则`**

---

### Task D2: critical 告警规则（5 条）

**Files:**
- Create: `ops/observability/prometheus/rules/critical-alerts.yml`

- [ ] **Step 1: 5 条 critical alert（spec §9.2 表）**
...
- [ ] **Step 2: 校验 `promtool check rules ...`**
- [ ] **Step 3: Commit `feat(obs): 5 条 critical 告警`**

---

### Task D3: warning 告警规则（11 条）

**Files:**
- Create: `ops/observability/prometheus/rules/warning-alerts.yml`

- [ ] **Step 1: 11 条 warning alert（spec §9.3 表）** — 按表逐条；burn-rate 两条单独放 D4。
- [ ] **Step 2: 校验**
- [ ] **Step 3: Commit `feat(obs): 11 条 warning 告警`**

---

### Task D4: 错误预算燃烧率告警（2 条）

**Files:**
- Create: `ops/observability/prometheus/rules/burn-rate-alerts.yml`

- [ ] **Step 1: 2 条 burn-rate alert**（1h 窗 14.4× / 6h 窗 6×）
...
- [ ] **Step 2: Commit `feat(obs): SLO 错误预算燃烧率告警（1h+6h 双窗口）`**
...
- [ ] **Step 3: Commit `test(obs): promtool 32 条 alert 单元测试（正反例）`**
...
- [ ] **Step 1: 撰文 — 来源 spec §9 + 实际 yml**
  - 4 个 SLO 解释（指标含义、目标、SLI 计算）
  - 16 条告警全表（severity / 表达式 / for / 通道 / runbook 锚点）
  - 静默与抑制使用方式（amtool / Grafana UI）
  - 客户视角："为什么这些数字是合同里写的？"
- [ ] **Step 2: 索引联动**
- [ ] **Step 3: Commit `docs(obs): SLO + 告警规则参考（Phase D 落实）`**
...
- [ ] **Step 2: dashboards.yml** — 标准 file provisioner 指向 `/var/lib/grafana/dashboards`
- [ ] **Step 3: slo-overview.json**（Grafana 11 schemaVersion ≥ 39，4 stat panel + 1 gauge + 2 timeseries + 1 table）
...
- [ ] **Step 5: Commit `feat(obs): Grafana provisioning + SLO Overview dashboard`**
...
- [ ] **Step 4: jq 校验三份 JSON**
- [ ] **Step 5: Commit `feat(obs): infra + jvm + http dashboards`**
...
ems-alarm：
- 活跃告警 by type / 检测耗时 / Webhook 成功率 / 事件流时间线

ems-meter：
- 数据新鲜度 by 设备 / 入库速率 by energy_type / 丢弃率

- [ ] **Step 4: jq 校验**
- [ ] **Step 5: Commit `feat(obs): collector + alarm + meter 业务 dashboards`**
...
- [ ] **Step 1: 撰文 — 来源 spec §10 + 7 个 JSON 真实面板说明**
...
- [ ] **Step 2: 索引联动**
- [ ] **Step 3: Commit `docs(obs): dashboards 使用指南（Phase E 落实）`**
...
- [ ] **Step 2: 推 PR 触发 CI 自验证（或本地 act 模拟）**
- [ ] **Step 3: Commit `ci: observability job（promtool + amtool + dashboards JSON + webhook-bridge build）`**
...
- [ ] **Step 1: 增加端到端 alert 注入测试**（追加在 5 服务 ready 之后）
...
- [ ] **Step 2: Commit `feat(obs): obs-smoke 端到端 alert 注入校验`**
...
- [ ] **Step 2: README 索引联动 + 各 alert 注解 runbook_url 锚点验证**
- [ ] **Step 3: Commit `docs(obs): runbook（Phase F 落实）`**
...
- [ ] **Step 1: 撰文 — 来源 spec §1-§4 + 既有 alarm-feature-overview 模板**
  - 一句话价值
  - 解决什么问题（4 条客户痛点）
  - 核心功能（6 条）
  - 适用场景 A/B/C 完整故事
  - 与 ems-alarm 的协同（业务告警 vs 基础设施告警）
- [ ] **Step 2: Commit `docs(obs): 功能概览（销售视角）`**
...
- [ ] **Step 1: 撰文 — 操作手册风**
  ...
  - 3.如何在收到告警邮件后定位
  - 4.如何在维护期暂停告警（amtool / Grafana UI 截图占位）
  - 5.如何申请 viewer 账号（链 §15 权限矩阵）
  - 6.FAQ × 8
  - 7.术语表
- [ ] **Step 2: Commit `docs(obs): 用户指南（客户运维视角）`**
...
- [ ] **Step 1: 撰文 — 来源 spec §8 + Prometheus 抓取协议**
...
- [ ] **Step 2: Commit `docs(obs): metrics API（集成方视角）`**
...
- [ ] **Step 1: 验收日志** — 复用 `verification-2026-04-29-alarm.md` 模板：
  ...
  - 客户验收 checklist
- [ ] **Step 2: 各 README 全量交叉链接**
- [ ] **Step 3: 根 README 商业化进度 sub-project #1 标 ✅**
- [ ] **Step 4: Commit `docs(obs): 验收日志 + README 索引大全（Phase G 落实）`**
...
- [ ] **Step 3: 通报**：在 ops 频道（手工）+ docs/ops/verification-2026-04-29-observability.md 末尾追加"v1.7.0-obs 已打 tag"。
...
- [ ] Spec §1-§7 价值/架构 → Phase A 应用接入 + Phase C 观测栈基础设施 全覆盖
- [ ] Spec §8 17 metrics → Phase B 4 个 metrics 类一一对应
- [ ] Spec §9 SLO + 16 alerts → Phase D 4+5+11+2 = 22 条规则覆盖（其中 11 warning 含 2 burn-rate 单独入 D4）
- [ ] Spec §10 7 dashboards → Phase E 4 task（E1=SLO+provisioning，E2=infra/jvm/http，E3=业务三件套，E4=guide doc）
- [ ] Spec §11 配置 → Task A4 + C1 + A5 doc
- [ ] Spec §12 测试策略 → 各 Task 内置 unit/promtool；Task F1 CI 集成；Task F2 smoke 端到端
- [ ] Spec §16 部署 → Task C6 docker-compose + scripts；Task C7 deployment doc
- [ ] Spec §17 日志规范 → 已通过既有 logback JSON + Loki/promtail 自动覆盖（无新代码）
- [ ] Spec §18 10 个文档产物 → A5 / B5 / C7 / D6 / E4 / F3 / G1 / G2 / G3 / G4 全 10 篇映射
- [ ] Spec §20 8 个不变量 → 上节 Key Invariants 完整复述
