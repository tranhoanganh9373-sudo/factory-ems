...
- **审计范围**：登录、看板、表计、组织树、报表、告警健康、采集、平面图、电价、用户管理（共 10 页）
...
- **修法**：① 第一行整行排 4 个状态卡片（在线/离线/告警/维护）—— `alarms/health` 页本身就是这 4 卡片，看板首屏可直接复用；② 空态用 AntD `<Empty>` 但替换图标为 `LineChartOutlined`/`DatabaseOutlined`，文案改"等待数据接入"+ 操作按钮"配置测点 →"。
...
- **P3-03** 顶栏图标行（主题/告警/用户）三者间距 16，建议 24，呼吸感更好。
