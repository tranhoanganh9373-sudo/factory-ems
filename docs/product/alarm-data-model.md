...
告警系统共涉及 5 张表，以及与既有业务表 `meters`（设备）和 `users`（用户）的软关联。
...
| 关联 | 类型 | 说明 |
|------|------|------|
| `alarms.device_id` → `meters.id` | 软关联，无 FK | 告警关联触发它的设备 |
| `alarm_rules_override.device_id` → `meters.id` | 软关联，无 FK（同为 PK）| 设备级阈值覆盖，一对一关系 |
| `webhook_delivery_log.alarm_id` → `alarms.id` | 软关联，无 FK | 记录每次告警的 Webhook 派发结果 |
...
| 字段名 | 类型 | 可空 | 默认值 | 业务含义 |
|--------|------|------|--------|---------|
| `id` | BIGSERIAL | 否 | 自动递增 | 告警唯一标识符，在整个系统中不重复，用于关联派发日志、站内通知等下游记录 |
| `device_id` | BIGINT | 否 | — | 触发本次告警的设备 ID，软关联 `meters.id`；结合 `device_type` 确定设备身份 |
| `device_type` | VARCHAR(32) | 否 | — | 触发告警的设备类型，首版取值：`METER`（电表）、`COLLECTOR`（采集器） |
| `alarm_type` | VARCHAR(32) | 否 | — | 告警触发原因类型：`SILENT_TIMEOUT`（设备长时间无数据上报）、`CONSECUTIVE_FAIL`（采集器连续失败达阈值） |
| `severity` | VARCHAR(16) | 否 | `'WARNING'` | 告警级别，首版固定为 `WARNING`；预留字段，Phase E 可扩展 `CRITICAL` / `INFO` |
| `status` | VARCHAR(16) | 否 | — | 告警当前状态，三值状态机：`ACTIVE`（未处理）→ `ACKED`（已确认）→ `RESOLVED`（已解决） |
| `triggered_at` | TIMESTAMPTZ | 否 | — | 告警首次触发的时刻，是整个告警生命周期的起点，用于 SLA 计算和报表统计 |
| `acked_at` | TIMESTAMPTZ | 是 | NULL | 运维人员确认告警的时刻；`NULL` 表示尚未确认；与 `triggered_at` 之差即"响应时延" |
| `acked_by` | BIGINT | 是 | NULL | 执行确认操作的用户 ID，软关联 `users.id`；用于审计"谁确认了这条告警" |
| `resolved_at` | TIMESTAMPTZ | 是 | NULL | 告警消除的时刻；`NULL` 表示尚未解决；与 `triggered_at` 之差即"故障持续时长" |
| `resolved_reason` | VARCHAR(32) | 是 | NULL | 告警消除方式：`AUTO`（检测引擎自动判定设备恢复）、`MANUAL`（运维人员手动关闭） |
...
| 索引名 | 字段 | 服务的查询场景 |
|--------|------|--------------|
| `idx_alarms_device_status` | `(device_id, status)` | 查询某台设备的所有 ACTIVE 告警（最高频场景：监控大屏、设备详情页） |
| `idx_alarms_status_trig` | `(status, triggered_at DESC)` | 按状态+时间倒序分页列出告警（告警列表页主查询） |
| `idx_alarms_triggered_at` | `(triggered_at DESC)` | 不过滤状态、仅按时间范围查告警（月报统计、历史回溯） |
...
| `maintenance_mode` | BOOLEAN | 否 | `FALSE` | 维护模式开关；`TRUE` 时检测引擎跳过该设备所有检测，不产生任何告警，适用于计划停机、设备检修期间 |
...
| `enabled` | BOOLEAN | 否 | `FALSE` | Webhook 总开关；`FALSE` 时系统不发送任何 Webhook，即使告警产生也不触发推送 |
...
| `alarm_id` | BIGINT | 否 | — | 本次派发对应的告警 ID，软关联 `alarms.id`；可通过此字段反查是哪条告警触发了这次推送 |
...
| 索引名 | 字段 | 服务的查询场景 |
|--------|------|--------------|
| `idx_wdl_alarm` | `(alarm_id)` | 通过告警 ID 快速查找对应的派发记录（告警详情页展示推送状态） |
...
| `alarm_id` | BIGINT | 否 | — | 本条通知关联的告警 ID，软关联 `alarms.id`；用户点击通知时通过此字段跳转到告警详情 |
| `user_id` | BIGINT | 否 | — | 通知收件人用户 ID，软关联 `users.id`；每个用户各收一条，不共享消息记录 |
| `kind` | VARCHAR(16) | 否 | — | 通知类型：`TRIGGERED`（告警产生）、`RESOLVED`（告警解决）；决定前端显示的图标和文案 |
...
|------|------------|------------|---------|
| `alarms` | 永久保留 | 历史告警具有审计与合规价值，建议保留 ≥ 2 年；2 年以上可归档至冷存储或数据仓库 | 暂无 |
...
### 场景 3：某设备最近一次告警的完整生命周期

排障时查看指定设备最新告警从触发到解决的全部字段，包括触发上下文快照。
