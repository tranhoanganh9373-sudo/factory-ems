...
Factory EMS 是一个模块化能源管理平台。沿能源数据流向看，它做四件事：
...
这四件事背后是 16 个 Maven 模块（`ems-*`），分四层组织：数据底座、业务领域、平台能力、启动。
...
支持任意层级的树形结构，每个节点带有面积、人数等元数据，用于后续分摊。所有报表、账单、报警都按节点聚合，选了"一车间"，就汇总该节点下所有仪表。
...
- **采集属性**：采集协议、设备地址、点位映射、`channelPointKey`（通道侧测点 key）
- **量值类型**（v1.1.0+）：`INTERVAL_DELTA` / `CUMULATIVE_ENERGY` / `INSTANT_POWER`，决定时序聚合算子（详见 [meter-feature-overview.md](./meter-feature-overview.md) §3）
- **业务属性**：超时阈值（多久没收到数据算断线）、连续失败次数阈值、是否在维护期
- **状态**：启用 / 停用 / 维护中；列表页「采集状态」列直接显示真实在线/离线/维护中（基于 `DiagnosticRingBuffer.lastGoodSampleAt` + 5 分钟新鲜度窗口，与报警健康卡同口径）

支持**仪表树**视图，配合组织树呈现"哪些设备挂在哪个节点下"。

**典型用法**：
- 设备级阈值：主变压器超时 30 秒就报警，照明回路超时 5 分钟才报警
- 维护模式：提前给某台仪表开维护模式，停机检修期间不出报警
- 查阈值历史：审计日志能查出"谁在什么时候改了哪台仪表的阈值"
- **批量导入**（v1.1.0+）：实施现场把仪表台账 CSV 一次拖入，前端预览后逐条入库（同名跳过）
...
- **4 个 SLO**：API 可用性、采集完整率、报警时延、报表准时率
- **7 个 Grafana 看板**：API 健康、采集层、报警链路、报表与导出、数据库、Web、SLO 总览
- **16 个 Prometheus 报警规则**：关联到 4 个 SLO，可路由到值班 / 一线 / 客户
...
| 角色码 | 名称 | 范围 |
|---|---|---|
| `ADMIN` | 管理员 | 全部权限，含用户管理、采集配置、报警配置、关账解锁 |
| `OPERATOR` | 运维 | 看报警、确认 / 关闭报警、维护模式开关 |
...
7. **接报警**：进 `/alarms/webhook`，配企业微信 / 钉钉 Webhook 地址，做一次断线测试
...
1. **收到报警**：钉钉 / 企业微信收到 Webhook 推送，或站内铃铛角标提醒
2. **看健康总览**：进 `/alarms/health`，定位是哪台设备
3. **现场处置**：到现场恢复网线 / 重启网关
4. **确认报警**：回到 `/alarms/history`，把对应报警标记"已确认"
5. **等自动恢复**：数据流恢复后，报警状态自动转"已解决"
6. **如计划停机**：提前进 `/meters` 给相关设备开维护模式，期间不出报警
