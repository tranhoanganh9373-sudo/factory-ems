...
把"什么时间、哪个节点、哪种能源、按什么粒度统计"这件事固化成系统能力。运营要日报、财务要月报、管理要年度审查、班组要班次能耗，都对应到一处接口。同时支持自定义查询（ad-hoc）和大数据量异步导出 Excel / PDF。
...
每月初做绩效考核。在 `/report/preset/shift` 选上月、节点选"包装车间"，得到每天每班一行"产量 + 能耗 + 单位产量能耗"。透视后发现"夜班 5.2 度/件 < 早班 5.8 度/件"，于是夜班奖励、早班排查。
...
报表既消费各模块的数据，也再加工：把多源数据按业务口径揉成可读形式。
