...
- **Modbus 自动重连退避**：连接失败后不会自动重试；需点「重连」按钮或重启服务（MQTT / OPC UA 由底层 SDK 处理）
...
> 已在 v1.1 落地（先前文档曾标记为未实装，现已交付）：OPC UA `SIGN` / `SIGN_AND_ENCRYPT` 模式、服务端证书审批 REST + 前端审批页（`/admin/cert-approval`）、`OPC_UA_CERT_PENDING` 自动告警 / 自动解除、MQTT QoS 2 + Last Will。
