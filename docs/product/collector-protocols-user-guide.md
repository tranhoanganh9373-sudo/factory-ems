...
| OPC UA 报 `Untrusted server certificate` | 服务器证书未在 trusted 目录 | 已自动落入 `pending/` 并触发告警；ADMIN 在 `/admin/cert-approval` 批准后自动重连，详见 [opcua-cert-management.md §5](../ops/opcua-cert-management.md) |
...
> 已在 v1.1 落地（先前文档曾标记为未实装，现已交付）：OPC UA `SIGN` / `SIGN_AND_ENCRYPT` 模式、服务端证书审批 REST + 前端审批页（`/admin/cert-approval`）、`OPC_UA_CERT_PENDING` 自动告警 / 自动解除、MQTT QoS 2 + Last Will、Modbus TCP/RTU 指数退避自动重连、通道删除按钮（`/collector` 列表行内 `删除`，含确认弹窗）。
