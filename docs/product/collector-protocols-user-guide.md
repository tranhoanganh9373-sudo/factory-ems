...
> **适用版本**：v1.1（CP-Phase 1-9 完成时） ｜ 最近更新：2026-05-04
> **受众**：现场运维 / 工程实施 / 客户管理员
...
所有协议都在前端 `/collector` 页通过统一的 ChannelEditor Drawer 配置。保存后后台 `ChannelService` 启动 Transport，5 秒内会出现在实时状态表中。
...
| PLC 已开放 OPC UA endpoint | `OPC_UA` | 全部三种 SecurityMode 已端到端可用：`NONE` / `SIGN` / `SIGN_AND_ENCRYPT`（后两者需上传 `.pfx` 客户端证书） |
| 边缘网关把数据推到 MQTT broker | `MQTT` | broker 单向上报，无需 EMS 反向连仪表 |
| 新车间硬件未到货 / 内部演示 | `VIRTUAL` | 4 种模拟模式，下游链路一并联调 |

> **`OPC UA` 订阅模式（SUBSCRIBE）已端到端打通**：在测点上把 `mode` 设为 `SUBSCRIBE` 即可启用，由 `OpcUaTransport.setupSubscription` 走 Eclipse Milo `ManagedSubscription` 推送。同一 channel 可混合 `READ` + `SUBSCRIBE` 测点。
...
### 3.4 批量 CSV 导入通道（v1.1.0+）

实施场景下需要一次创建几十至上百条通道时，逐条 Drawer 太慢。`/collector` 页右上角「批量导入」按钮接受**两个 CSV 文件**：通道清单 + 测点清单。

**两份 CSV 的角色**：

- `channels.csv`——通道本体连接参数。表头：`name, protocol`（必填）+ 协议特定列：`host` / `port` / `serialPort` / `baudRate` / `parity` / `endpointUrl` / `securityMode` / `brokerUrl` / `pollInterval` 等
- `points.csv`——每条通道下挂的测点。表头：`channelName, key, address, dataType`（必填）+ 协议特定列：`functionCode` / `nodeId` / `topic` / `jsonPath` 等。`channelName` 列把测点关联回通道

**导入流程**：

1. 在 Excel 整理两个 sheet，分别另存为 UTF-8 编码的 CSV
2. `/collector` 页点「批量导入」→ 上传两份文件
3. 后端 `POST /api/v1/channel/parse-csv` 仅校验并返回 `ChannelDTO[]` 解析结果，**不立即落库**
4. 前端弹窗以表格形式展示每条解析后的通道（带 `name / protocol / 测点数 / 状态`），用户确认后再逐条 `POST /api/v1/channel` 创建
5. 同名通道自动跳过（标"已存在"），失败行标"失败"显示原因——其余行正常入库
6. 落库后每条通道自动启动 Transport，5 秒内出现在实时表

**为什么是两份 CSV**：通道与测点是 1-N 关系，单文件难以表达多测点结构；分两份的同时也便于客户在 Excel 端独立维护「设备表」和「点位表」两个 sheet。

**API 端点**：详见 [docs/api/collector-api.md](../api/collector-api.md) §1.7。仅 ADMIN 可见。
...
> 所有 VIRTUAL 采集生成的 `Sample` 自动带 tag `virtual=true`，下游报警 / 对账可按需过滤。
...
| OPC UA 报 `Untrusted server certificate` | 服务器证书未在 trusted 目录 | 已自动落入 `pending/` 并触发报警；ADMIN 在 `/admin/cert-approval` 批准后自动重连，详见 [opcua-cert-management.md §5](../ops/opcua-cert-management.md) |
...
| 状态卡在 ERROR 不重试 | Modbus 已自动指数退避重连（1s→60s 封顶）；如长期 ERROR 通常是仪表侧问题 | 检查仪表上电 / 网线 / 串口；可点「重连」按钮强制立刻重试 |

---

## 7. 已知限制（v1.1）

以下能力在 v1.1 **暂未实装**，将在 v2 评估：

- **`.pfx` 客户端证书 multipart 上传**：spec §8.3 描述的 `POST /api/v1/secrets/opcua/cert` 未实装；需手工把 PEM/PFX 放入 `~/.ems/secrets/opcua/clients/`，再通过 `POST /api/v1/secrets` 写引用

> 已在 v1.1 落地（先前文档曾标记为未实装，现已交付）：OPC UA `SIGN` / `SIGN_AND_ENCRYPT` 模式、服务端证书审批 REST + 前端审批页（`/admin/cert-approval`）、`OPC_UA_CERT_PENDING` 自动报警 / 自动解除、MQTT QoS 2 + Last Will、Modbus TCP/RTU 指数退避自动重连、通道删除按钮（`/collector` 列表行内 `删除`，含确认弹窗）、**通道批量 CSV 导入**（§3.4）、**Modbus RTU/TCP 心跳与错误展开**（重连不再误报"采集成功"，需到 transport 握手成功才算）、**DiagnosticRingBuffer**（每通道 100 条最近样本环形缓冲，供 `lastGoodSampleAt` 判定真实在线状态——给报警与表计列表共用）。
