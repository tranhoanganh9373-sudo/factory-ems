...
把工厂的车间平面图（CAD 转 PNG / JPG）上传到系统，再把每块仪表拖到底图上对应位置。从此，运维一打开页面就能看到全场仪表的实时状态和读数——红黄绿三色一目了然，断线哪个、漏水哪里、能耗高在哪个区域，都不用查列表。
...
- **新人不熟悉现场**：靠口头讲解和翻施工图，耗时长且记不住，看到告警不知道仪表在哪。
...
- **实时态显示**：每个挂点实时显示读数（kW / kvarh / m³ / kg）+ 在线状态色（绿色 = 在线、黄色 = 即将告警、红色 = 离线 / 告警）。
...
ems-floorplan
       ...
       ├──────< ems-alarm        挂点变红色受告警状态驱动
       └──────> ems-dashboard    /floorplan/{id}/live 端点供仪表盘嵌入
```

平面图是"运维 + 演示"两用层——既给运维快速定位，又给客户做视觉化展示。
