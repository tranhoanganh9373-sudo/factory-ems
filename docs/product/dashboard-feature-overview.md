...
登录后第一眼看到的"全场能耗驾驶舱"——今天用了多少电、当前需求功率多少、能耗集中在哪些区域、采集健康率、单位产量能耗趋势、各电价时段占比、Top 设备、平面图实时态。所有 KPI 一屏呈现，不用切多个页面凑信息。
...
- **数据散落**：能耗在 InfluxDB、电价在 PostgreSQL、产量在产量表、告警在告警表——一屏聚合需要后端代劳。
...
- **§3.1 KPI 摘要卡（`/kpi`）**：今日累计用电、当前需求功率、采集健康在线率、活跃告警数、单位产量能耗。一屏顶部 4-5 张卡片。
...
- **§3.5 电价分布饼（`/tariff-distribution`）**：本月各电价档位（峰 / 平 / 谷 / 尖峰）的电费占比 — 帮助识别"夜间用电太少导致谷价红利没吃到"等异常。
...
- **§3.10 仪表详情页（`/meter/{id}/detail`）**：点 Top N 或 Sankey 下钻到单表详情，看该表近期的功率曲线 + 累计电量 + 历史告警。
...
进 `/dashboard`，先看 KPI 摘要卡：今日已用 8500 度（同比 +5%、环比 -3%）、当前功率 850 kW、采集在线率 98.5%、活跃告警 2 条。然后看 Top N 是否有反常（突然出现的高耗设备）。30 秒看完，正常就关页面，异常就下钻到具体仪表。
...
打开 `/dashboard` 节点选"全厂"，主屏放 Sankey 桑基图 + 平面图实时态。观众一眼看到全场能源流向 + 设备实时态，比 Excel 报表震撼得多。
...
- **不做 KPI 阈值告警**：仪表盘只展示数据，不做"今日用电 > 阈值就发短信"这种业务规则。这类规则规划在 v2.x。
...
ems-alarm              KPI 卡里的"活跃告警"数
       │
       └────> ems-dashboard（聚合 + 渲染层）
```

仪表盘是"消费者"——它不存数据，所有数据来自其他模块。这意味着前置模块的数据质量直接决定仪表盘可用性。
