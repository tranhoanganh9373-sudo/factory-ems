...
把"总表读到的电费"按业务规则分摊到各车间 / 各产线 / 各成本中心。支持三种算法（直分 / 按权重 / 残差）+ 组合策略。一次月结跑完，每个节点都拿到自己应承担的电费 + 来源明细，财务对账有据。

---

## §2 解决什么问题

- **总表只有一个数，分到各车间靠手算**：总表读 30000 度，要分给一车间 12000 度、二车间 10000 度、办公区 8000 度——靠 Excel 手工拉表，每月做一次累、易错。
- **分摊规则混合多种**：有些子节点有自己的子表（直分）；有些没子表只能按面积 / 人数 / 产值（按权重）；剩下没分到的算总表的"残差"挂到一个公共节点（残差）。一套规则要混合三种算法。
- **分摊结果要可追溯**：财务问"这 5000 度怎么算出来的"，得知道用的是哪条规则、哪些子表 / 权重、当时单价多少——口头交不清楚，要存在数据库里。
...
- **不做权重自动校正**：如果三个权重加起来不是 100%（如 30/30/30 = 90%），系统会按比例归一化但不告警。需要严格控制时人为校验。
...
成本分摊是"账单和成本报表的上游"——账单按节点 → 找该节点应承担的费用 → 来自 cost 的运行结果。
