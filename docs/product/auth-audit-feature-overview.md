...
ems-auth 管"谁能登录、能做什么、能看哪些节点"，包括账号、密码、JWT、4 个角色、节点级权限。ems-audit 管"谁做过什么"，所有写操作（建账期、关账、改电价、改报警等）都记一行日志，含操作人、时间、变更内容。两个模块合起来构成权限和留痕的安全底座。

---

## §2 解决什么问题

- **多人多角色，权限要分清**：财务能看费用、运营能改报警、查看者只读、管理员通杀。一套 RBAC + 节点级权限范围。
- **总部 / 厂区数据要隔离**：各厂区的运营只看自己厂区，不跨厂查别人数据。靠"组织节点权限"做范围切。
- **关键操作要留痕**：电价调整 / 账单锁定 / 用户改密 / 报警确认这些都是审计关注事项，过后查"谁、何时、改了什么"必须有据。
...
- **OPERATOR**：日常运营，包括报警处理、平面图编辑、采集配置、产量录入。
...
- **不做审计实时报警**：审计日志只记录、不推送（如"管理员凌晨改了电价"不会发短信报警）。
...
- 报警（确认 / 静默进审计）：[alarm-feature-overview.md](./alarm-feature-overview.md)
