...
**ems-auth** 管"谁能登录、能做什么、能看哪些节点"——账号、密码、JWT、4 角色、节点级权限。**ems-audit** 管"谁做过什么"——所有写操作（建账期、关账、改电价、改告警等）记一行日志，含操作人、时间、变更内容。两模块合起来构成"权限 + 留痕"的安全底座。
...
- **OPERATOR**：日常运营——告警处理、平面图编辑、采集配置、产量录入。
- **FINANCE**：财务——电价、账单、成本分摊、报表。
- **VIEWER**：只读——能看，不能改。
...
ems-auth 是"所有请求的入口闸门"；ems-audit 是"所有写操作的留痕仓库"。两模块都是横切，被 16 个模块依赖。
