...
本字典描述 factory-ems 应用层注册的 **17 个业务 metrics**（Phase B），涵盖采集、告警、计量、应用跨模块四个维度。

**怎么用这份文档：**

- 按模块快速定位 → 查"模块清单"表（第 3 节）
- 按指标名搜索 → 用浏览器 Ctrl+F 搜索名称（如 `ems.collector.poll.duration`）
- 想写 PromQL → 直接复制"典型 PromQL"列的示例，按需替换 label 值
- 想加新指标 → 阅读第 8 节"新增指标 checklist"
...
所有业务指标和 Spring Boot Actuator 默认指标都会自动附带以下两个公共标签，由 `ObservabilityConfig` 通过 `MeterRegistryCustomizer` 在注册表级别注入：
...
**实施差异（spec §8.1）**：spec 文本中提到了 `module` 标签（从包名派生），但当前 v1 实现中 `ObservabilityConfig` 仅注入 `application` 和 `instance` 两个公共标签，**`module` 标签未注册**。若需区分模块，请在 PromQL 中按指标名前缀过滤（`ems_collector_*`、`ems_alarm_*` 等）。
...
**实施差异（粒度说明）**：spec §8.2 描述为"单次寄存器读"，但 v1 实现中 `DevicePoller` 的 poll 循环结构是"一次 cycle = 多寄存器顺序读"。实际粒度为 **cycle 级**：一次 cycle 所有寄存器全部成功 → `read.success.total +1`；cycle 失败（重试耗尽）→ `read.failure.total +1`。对 `rate()` 告警规则的语义无影响（详见 `CollectorMetrics.java` javadoc）。
...
**实施差异（dropped reason）**：v1 `InfluxReadingSink` 仅在"仪表未找到（meter == null）"时记录 `reason=other`。`duplicate` / `out_of_range` / `format_error` 路径在校验链路完善后（Phase D+）方可触发，枚举值已预留。
...
`action`（审计操作类型）：来自 `AuditEvent.action()` 的字符串（如 `CREATE_ALARM`、`RESOLVE_ALARM`、`LOGIN` 等）；空值归一化为 `other`。**非固定枚举**，随业务扩展增长，cardinality 受业务操作类型数量控制（预计 < 30）。

`type`（异常类型）：来自 `ex.getClass().getSimpleName()`（如 `NullPointerException`、`IllegalArgumentException`）；**仅计 unhandled 异常**（`BusinessException`、`AccessDeniedException` 等业务预期异常在专属 handler 处理，不进入兜底分支）；空值归一化为 `other`。
...
新增 `@Scheduled` 方法会被 AOP 自动捕获，无需手动注册；`task` 值会自动增长，预计 < 50。
...
高基数 label 是 Prometheus 的主要资源杀手。factory-ems 遵循以下规则（参见 spec §8.6）：
...
v1 阶段工厂设备规模预计 **< 5000 台**，cardinality 在可控范围内（success：5000 序列；failure：5000 × 5 reason = 25000 序列）。如设备规模增长，优先评估是否需要降采样或聚合录制规则。
...
所有 label 值在埋点处统一做归一化（`KNOWN_*` set 检查 + `other` 兜底），**禁止将用户输入、API 参数或不可控外部字段直接用作 label value**，防止 cardinality 爆炸（详见 spec §8.6 cardinality 控制）。

---

## 6. Spring Boot Actuator 默认指标

以下指标由 Spring Boot Actuator + Micrometer 自动注册，**无需重复埋点**，在 Grafana dashboard 中直接使用（参见 spec §8.7）：
...
这些指标覆盖了 JVM 健康、HTTP SLO、数据库连接池、日志异常率四个维度，与业务指标互补。
...
spec §8.5 定义了 `ems.app.scheduled.drift.seconds`（Gauge，`{task}`），用于追踪调度任务实际触发时间与期望触发时间之间的绝对偏差。

### 7.2 v1 现状（deferred）

**v1 应用层不注册此 Gauge**。`AppMetrics.java` 中已有注释说明。

原因：

1. `SchedulerInstrumentationAspect` 仅能在 `@Around` 中拿到任务执行的实际开始时刻，无法直接获取 Spring Scheduler 的期望触发时刻（`@Scheduled(fixedRate=...)` 的下次期望时间未在 Spring Context 公开 API 中暴露）
2. 对单机 on-prem 部署，Phase D 通过 PromQL 基于 `ems_app_scheduled_duration` timer 的时间序列推导漂移，无需应用侧 Gauge
...
如后续需要应用层精确漂移 Gauge，在 `SchedulerInstrumentationAspect` 中扩展：
...
> Spring 的 `TaskScheduler` 不直接暴露"期望下次触发时刻"，需在注册时持有固定 rate/delay 配置自行计算。

---

## 8. 新增指标 Checklist

在 factory-ems 中新增业务指标，请遵循以下 7 步：

1. **命名约定**：使用 `ems.<module>.<concept>.<unit>` 格式
   - 正例：`ems.collector.poll.duration`、`ems.alarm.created.total`
   - 禁止重复已有名称；禁止在名称中包含 label 值（如不写 `ems.alarm.timeout.count`，用 `reason=timeout` label 代替）

2. **Label 上限**：每个指标的 label 数量 **≤ 5**；高基数 label（如设备 ID 类）需提前评估最大基数
...
4. **归一化未知值**：为每个枚举型 label 维护 `static final Set<String> KNOWN_*`，未匹配的值统一归为 `other`

5. **加单元测试**：至少验证：正常路径递增、未知 label 归一化为 `other`、`NOOP` 构造函数不抛异常

6. **更新本字典**：在第 4 节对应模块中增加一行，填写名称、类型、单位、Labels、含义、数据更新方式

7. **评估 cardinality**：估算新 label 的最大基数，与现有序列数叠加后确认总量仍在 Prometheus 服务器资源预算内（参见 spec §8.6 cardinality 控制）

---

## 9. 常用 PromQL 模板

以下模板可直接在 Grafana Explore 或告警规则中使用：
