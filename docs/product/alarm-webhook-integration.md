...
以下情况无需配置 Webhook，站内通知（alarm_inbox）已满足需求：

- 值班人员全天在 EMS UI 内监控，无需外部推送
- 仅需记录告警历史，不需要实时触达
- 接收方需要双向交互（Webhook 是单向推送，不支持从接收方回调 EMS）

### 替代方案

EMS 默认为每条告警写入**站内 alarm_inbox 通知**，无需任何配置即可在 EMS UI 的"系统健康 → 告警历史"和"通知中心"查看。Webhook 是站内通知的**增强补充**，二者同时生效。

> **首版说明**：恢复事件（`alarm.resolved`）目前仅触发站内通知，不发送 Webhook 推送。
...
点击后，EMS 使用**当前保存的配置**向接收方发送一条 `alarm.test` 事件：

- 无需创建真实告警，使用占位 Sample Alarm 数据
...
`enabled=false` 时，EMS 完全跳过 Webhook 派发逻辑，所有告警事件仍正常写入站内通知。
...
所有时间戳均为 **ISO 8601 / RFC 3339 格式，含时区偏移**（例：`2026-04-29T08:15:30+08:00`），字符集 **UTF-8**。
...
> **注意**：首版 Webhook **不发送 `alarm.resolved`**（恢复事件仅写入站内 alarm_inbox 通知）。
...
`X-EMS-Event` 字段提供给接收方在**无需解析 Body** 的情况下即可知道事件类型，便于：
...
**绝对不要**用字符串直接 `==` 比较签名。各语言完整验签实现详见 §5.6。

### 5.2 必须幂等

EMS 重试机制可能导致同一事件**多次到达**接收方。推荐去重策略：
...
建议接收方在 **1 秒内返回响应**（默认 `timeout_ms=5000`）。如果接收方需要执行耗时操作（如调用外部 API），建议先返回 `200 OK`，再异步执行后续处理（写队列、后台任务等）。
...
在"系统健康 → Webhook 配置 → 发送记录"中找到 `FAILED` 行，点击**"重发"**按钮（仅 ADMIN 角色可见）。
...
2. 点击**"测试发送"**按钮
3. EMS 使用当前保存配置发送 `alarm.test` 事件（使用最近一条 ACTIVE 告警数据；如无，则使用占位 Sample Alarm）
...
2. 在 EMS Webhook 配置页将 `url` 填写为上述临时 URL，`secret` 随意设置（如 `test-secret-123`）
3. 点击**"测试发送"**
4. 在 webhook.site 页面查看完整的 Headers 和 Payload，验证字段是否符合预期
5. 调试完成后，将 `url` 改回真实接收端

### 8.3 curl 模拟验签

使用 `curl` 手动模拟 EMS 发送，验证自己的接收端签名实现：
...
Secret 应至少为 **32 字符随机串**。生成命令：

```bash
openssl rand -hex 32
```

避免使用：固定词汇、UUID、可猜测的字符串。

### 9.2 HTTPS 强烈推荐

HTTP 明文传输会导致：

- Payload 中的设备信息、告警时间泄露
- 中间人可伪造 Webhook 请求（虽有签名保护，但明文暴露更多上下文）

建议接收方必须部署 TLS 证书，`url` 字段以 `https://` 开头。
...
除 `X-EMS-Signature` 验签外，可在 URL 中附加 Token 参数做额外校验：
...
建议**每季度**轮换一次 Secret。轮换步骤：

1. 接收方**准备好新 Secret** 但继续使用旧 Secret 验签（不切换）
2. 在 **EMS UI** 的 Webhook 配置页将 Secret 更新为新值（即时生效）
3. EMS 从此刻起使用新 Secret 签名
4. 接收方切换验签 Secret 为新值
5. 弃用旧 Secret
...
如需直接在 EMS 内集成新的消息格式（无需中间适配层），可按以下步骤扩展：
...
标注 `@Component` 后，Spring 自动将其注册到 `Map<String, WebhookAdapter>` Bean（按 `getType()` 返回值为 key）。`WebhookChannel` 按 `webhook_config.adapter_type` 字段路由到对应实现，**无需修改主派发流程**（攻击面隔离）。

### 步骤 3：前端下拉选项

在 Webhook 配置页的 `adapter_type` 下拉框中新增该选项（值与 `getType()` 返回值一致）。
...
在本文档 §6 下新增对应的 `6.X` 子节，描述该 Adapter 的消息格式和使用方式。
