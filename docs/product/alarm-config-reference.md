...
除了全局默认值，系统支持对单个设备运行时覆盖阈值，不用重启。两种设置方式：

1. API 方式：`PUT /api/v1/alarm-rules/overrides/{deviceId}` 
2. UI 方式：登录后进入 系统健康 → 阈值规则，在"设备级覆盖"卡片中找到目标设备编辑
...
覆盖修改立即生效（不用重启）。设定值在下一轮检测周期（最长等 60s）应用。如果设置了多个覆盖值，系统按以下优先级：
1. 设备级覆盖（最高优先）
2. 全局默认（最低优先）
...
下面三种场景是工业现场的典型应用，对应的 YAML 段落可以直接复制到 `application.yml` 使用。
...
适用于中等规模工厂。平衡可靠性和系统负荷，是推荐的生产环境默认值。
...
适用于采集周期长（5-60 分钟）的设备。参数宽松，避免网络抖动产生误报。
...
**场景**：线上生产环境中，某个关键电表（设备 ID=88）网络不稳定，想临时放宽它的连续失败阈值。

**操作流程**：
1. 访问 UI：系统健康 → 阈值规则 → 搜索设备 88
2. 编辑该设备，设 `consecutive_fail_count` 为 5（原全局值为 3）
3. 提交保存
4. 等最多 60 秒，下一轮检测周期就会应用新值
5. 要恢复为全局默认，删除该覆盖即可

不需要改 `application.yml`，也不用重启应用。

---

## 5. 校验失败处理

应用启动时会校验 `application.yml` 中的 alarm 参数。如果参数不合法，应用会 fail-fast，日志输出详细错误信息。下表列出常见校验错误及修复方法。
