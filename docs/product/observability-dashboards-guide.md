...
- 黄色（25% ~ 50%）：预算消耗过半，建议放慢变更节奏，留意是否有持续的小故障。
- 红色（< 25%）：预算快耗尽；如果降到 0 本月 SLO 就违约了，应冻结非紧急发布。

基于 99.5% 目标，30 天允许的不可用总时间约为 216 分钟（约 3.6 小时）。
...
- 容器重启数 ≥ 3 → 跳 D3 JVM dashboard，查 Heap 是不是触顶（OOM 重启）或 GC 停顿过长。
- CPU 持续 > 90% → 跳 D4 HTTP dashboard，查 RPS 是不是异常激增，看 Top 10 慢端点。
...
- Heap 红了 → 看面板 12（Logback 日志），确认 error 是不是激增 → Loki Explore 过滤 `level=error` 查 OOM 堆栈。
- GC 停顿过高 → 看面板 8（GC 次数速率），G1GC 触发频繁通常和 Heap 使用率 > 90% 同时出现。
...
- p99 延迟红了 → 看面板 9（Top 10 慢端点），定位受影响的 URI → 跳 D3 JVM，查连接池是不是耗尽。
- 5xx 错误率升高 → 看面板 10（Top 10 高错误率端点），确认受影响范围 → Loki Explore 过滤对应 URI 查错误堆栈。
...
本 dashboard 排查时通常从这里开始：

- `device_id` 列：设备唯一标识，可与运维台账对应，找到物理设备位置。
- `fail_rate (per s)` 列：该设备每秒平均读取失败次数，越高越紧急。
- Loki 跳转链接：点每行右侧的"查看 device 日志 (Loki)"，会跳到 Loki Explore，预填 `device_id=<值>` 过滤条件，不用手工输入。

**失败率 by adapter（面板 4）**

按通信协议适配器（如 `modbus_tcp`、`modbus_rtu`）分色展示失败率。某一适配器失败率突然升高、其他适配器正常时，通常是该协议设备组批量故障或网络隔离，而不是采集服务整体异常。
...
- Loki 日志里出现 traceId → 点 traceId → Tempo 追踪详情 → 定位是哪个 modbus 适配器卡住、哪一步调用超时。
- 失败率 by adapter 中某适配器突增 → 对照 D1 SLO Overview 的"数据新鲜度" stat，看是不是已影响 SLO。
...
v1 阶段大多数丢弃原因显示为 `other`，这是预期行为（v1 丢弃原因分类还没细化）。`other` 之外的类别出现且持续增长时，要排查对应业务逻辑。
...
- Lag 红了 → 看面板 5（入库速率 by energy_type），定位哪种能源类型入库速率掉了 → 跳 D5 Collector，查该类型设备的失败率和 Top 10 失败设备。
- 丢弃总数持续升高 → 看面板 6（丢弃 by reason），确认丢弃原因类别 → Loki Explore 过滤 `"dropped"` 关键字查具体日志。
...
每个 dashboard 顶部都有 $instance 下拉变量，动态枚举所有向 Prometheus 注册的 `factory-ems` 实例：

- 默认值是 All，同时展示所有实例的聚合数据。
- 多服务器场景：部署了多个 factory-ems 节点（主备或分地域）时，下拉里选具体实例，过滤出单节点视图，方便对比或独立排查。
- URL 自动更新：切换 $instance 后浏览器 URL 的查询参数会同步更新，把这个 URL 分享给同事，对方打开就是相同的过滤视角。

### 时间窗口（Time Picker）

所有 dashboard 默认时间窗口是过去 6 小时，刷新间隔 30 秒。
...
D1 SLO Overview 里的"可用性 (30d)" stat 面板，内部计算窗口固定 30 天，不受 time picker 影响；其他面板随 time picker 变化。
...
- "Lag vs SLO gauge" 量规指针在红区（接近 300s）。
...
- "失败率 by adapter"图中，`modbus_tcp` 适配器失败率从 0 突增到约 60%。
- "Top 10 失败设备"表里 8 台设备的 `device_id` 全是电表类型。

[截图：Collector Dashboard — Top 10 失败设备表，8 行均为电表设备 ID]

### 第 4 步：Loki Explore 查日志

点"Top 10 失败设备"表第一行的"查看 device 日志 (Loki)"链接，浏览器跳到 Loki Explore，自动预填：
...
确认：设备的 modbus TCP 连接持续超时（30s），就是电力读数停滞的直接原因。

[截图：Loki Explore — 过滤后的 timeout 错误日志，traceId 高亮可点击]

### 第 5 步：Tempo Trace 定位适配器卡点

点日志行里的 traceId，跳到 Tempo 追踪详情。

Trace 树状图显示调用链：采集服务 → Modbus TCP 适配器 → 等待 30s → 超时异常。阻塞点在适配器读取步骤，其余 span 都正常。根据 trace 关联的实例信息，确认该节点上的 modbus TCP 连接已阻塞，要重启对应采集线程或者检查网络交换机到电表的物理连通性。
...
v1 提供的 7 个 dashboard 都设为只读（`editable: false`），由 Grafana provisioning 管理。这是为了保证团队共享的视图定义和 Git 仓库一致，避免本地修改被下次部署覆盖。

不建议直接改这 7 个 dashboard。不同客户场景差异较大，建议 fork 一份再改。

### 推荐做法：Fork 后提交

1. **导出 JSON**：在 Grafana 中打开任一 dashboard，点右上角齿轮图标 → "JSON Model"，复制全部 JSON 内容。
2. **修改 JSON**：把 `uid` 改成新的唯一 ID（如 `my-collector-v2`），`title` 改成新名称，按需调整面板。
3. **提交到仓库**：把改完的 JSON 文件存到 `ops/observability/grafana/dashboards/<新文件名>.json`，提交 PR。
4. **生效方式**：Grafana 重启后（`docker compose restart grafana`）通过 provisioning 自动加载新文件，不用手工导入。

### 权限说明

v1 Grafana 使用自建用户体系（spec §10.6 + §15）：
- Admin / Editor 角色：可以在界面里创建和编辑自建 dashboard（非 provisioned）。
- Viewer 角色：只读，不能编辑任何 dashboard。
- Provisioned dashboard（本文档中的 7 个）对任何角色都是只读，界面编辑入口被禁用。
...
**症状**：打开 dashboard 后，面板长时间转圈或加载超时。

**排查步骤**：
1. 检查 Prometheus 抓取间隔配置（应为 15s）：`curl http://<prometheus>:9090/api/v1/targets` 确认 target 状态是 `up`。
2. 检查 Grafana 数据源超时设置：Grafana → Configuration → Data Sources → Prometheus → Timeout，建议 ≥ 60s。
3. Prometheus 自身响应慢的话，检查 Prometheus 容器内存有没有受限：`docker stats prometheus`。

### Panel 显示 No Data

**症状**：面板提示"No data"或空白，但时间范围和实例选择没问题。

**排查步骤**：
1. 确认指标是不是真的被采集，在 factory-ems 所在服务器运行：
   ```bash
   curl http://localhost:8080/actuator/prometheus | grep <metric_name>
   ```
   没有输出说明该指标未暴露，可能是服务没正常启动或配置问题。
2. 在 Grafana Explore → Prometheus 中直接输入 PromQL 查询，看是查询问题还是数据源连接问题。
3. 检查 Prometheus 抓取日志里有没有该 target 的 scrape 错误。
